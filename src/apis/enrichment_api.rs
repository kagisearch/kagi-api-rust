...
/// struct for typed errors of method [`enrich_search`]
...
pub async fn enrich_search(configuration: &configuration::Configuration, q: &str, r#type: &str) -> Result<models::EnrichSearch200Response, Error<EnrichSearchError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::EnrichSearch200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::EnrichSearch200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<EnrichSearchError> = serde_json::from_str(&content).ok();
