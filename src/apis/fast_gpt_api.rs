...
/// struct for typed errors of method [`fast_gpt`]
...
pub async fn fast_gpt(configuration: &configuration::Configuration, fast_gpt_request: models::FastGptRequest) -> Result<models::FastGpt200Response, Error<FastGptError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::FastGpt200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::FastGpt200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<FastGptError> = serde_json::from_str(&content).ok();
